...
$getLastFetchedTimestamp = $dbh4->query("SELECT fld_last_fetched_ts FROM `tbict_freshdesk` WHERE fld_last_fetched_ts IS NOT NULL AND fld_group_id = 33000215508 ORDER BY fld_last_fetched_ts DESC LIMIT 1");
...
foreach($pages as $page) {
	$url = 'https://creditinfoph.freshdesk.com/api/v2/search/tickets?page='.$page.'&query="(group_id:33000215508)'.$filter.'"';

	
	//$url = 'https://creditinfoph.freshdesk.com/api/v2/search/tickets?page='.$page.'&query="(group_id:33000215508)"';
	//$url = 'https://creditinfoph.freshdesk.com/api/v2/search/tickets?page='.$page.'&query="(group_id:33000215508)%20AND%20(created_by:>%272024-'.$mon.'-01%27%20AND%20created_by:<%272024-'.$mon.'-31%27)"';
		...
		foreach ($value as $o) {
			...
			$checkTicket = $dbh4->query("SELECT * FROM tbict_freshdesk WHERE fld_ticket_id = '".$o['id']."'");
				if (!$chkTkt = $checkTicket->fetch_array()) {
					//$saveTicket = $dbh4->query("INSERT INTO tbict_freshdesk_loc (fld_ticket_id, fld_subject, fld_description, fld_created_at, fld_updated_at, fld_status) VALUES ('".$o['id']."', '".$subject."', '".$description."', '".$createdDate."', '".$updatedDate."', '".$o['status']."')");
