...
$getLastFetchedTimestamp = $dbh4->query("SELECT fld_last_fetched_ts FROM `tbict_freshdesk` WHERE fld_last_fetched_ts IS NOT NULL AND fld_group_id = 33000215509 ORDER BY fld_last_fetched_ts DESC LIMIT 1");
...
foreach($pages as $page) {
	$url = 'https://creditinfoph.freshdesk.com/api/v2/search/tickets?page='.$page.'&query="(group_id:33000215509)'.$filter.'"';
	
	//$url = 'https://creditinfoph.freshdesk.com/api/v2/search/tickets?page='.$page.'&query="(group_id:33000215509)"';
...
if($info['http_code'] == 200) {
	...
	foreach ($obj as $key => $value) {
		foreach ($value as $o) {
			...
			echo "SELECT * FROM tbict_freshdesk WHERE fld_ticket_id = '".$o['id']."'";
			echo "<br>";

			$checkTicket = $dbh4->query("SELECT * FROM tbict_freshdesk WHERE fld_ticket_id = '".$o['id']."' AND fld_ctrlno IS NULL");
				if (!$chkTkt = $checkTicket->fetch_array()) {
					...
					//$saveTicket = $dbh4->query("INSERT INTO tbict_freshdesk (fld_ticket_id, fld_subject, fld_description, fld_created_at, fld_updated_at, fld_status, fld_due_date, fld_date_insert, fld_group_id) VALUES ('".$o['id']."', '".$subject."', '".$description."', '".$createdDate."', '".$updatedDate."', '".$o['status']."', '".$dueDate."', '".$currDate."', '".$o['group_id']."') ");
					// echo "TICKET SAVED";
					// echo "<br>";
				}
				else {
