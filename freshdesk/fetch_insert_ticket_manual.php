...
$url = 'https://creditinfoph.freshdesk.com/api/v2/tickets/357246"';
...
if($info['http_code'] == 200) {
    ...
    // foreach($obj as $item) {
        preg_match('/[a-zA-Z]{2}\d{6}/', $obj['subject'], $matches);
        $provider_code = $matches[0];
        
        $subject = preg_match("/\[CIC PROD]/", $obj['subject']);
        ...
        if($subject){
            ...
            // if($se_provcode == $provider_code){
                ...
                $check_if_filed = $dbh->query("SELECT * FROM tbprodtickets WHERE fld_subject = '".$obj['subject']."'");
                $cii=$check_if_filed->fetch_array();

                if($cii['fld_id']){

                    // echo $filename." IN TBPRODTICKETS - CIC DATABASE; <br>";
                    ...
                    echo $obj['id']. " - - ". $matches[0]." - " .$file_name. "  " . $date_created2 . " NOT IN DATABASE; <br>";

                    
                    echo "INSERT INTO tbprodtickets (fld_id, fld_provcode, fld_subject, fld_created_time, fld_transmittal_status) VALUES ('".$obj['id']."', '".$provcode."', '".$file_name."', '".$date_created2."', 0)";
