...
if ($_POST['sbtBatchUpload']) {
  ...
  if ($ext == 'CSV') {
    ...
    $target_dir = "files/excel/";
    $target_file = $target_dir.'UPL2400'.date("YmdHis").'.'.$ext;
    move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file);


    $sheetData = array_map('str_getcsv', file($target_file));
       
   

    if ($_POST['uploadType'] == '1') {
      $sql = "INSERT INTO demo (id, trainingdate, firstname, initial, surname, gender, email, fld_provcode, fld_insert_ts, fld_timestamp, fld_phase, fld_company, fld_office, fld_nationality, fld_designation, fld_contact, fld_how, fld_is_registered, fld_orientation_attend, fld_training_attended_before, fld_training_attended_before_date, fld_id_link, fld_confirmation_message, fld_suffix) VALUES ";
      $err_date = 0;
      foreach ($sheetData as $key => $value) {
        if ($key >= 2 && !$err_date) {
          $schedule = date("Y-m-d", strtotime($value[15]));
          $fname = strtoupper($value[7]);
          $mname = strtoupper($value[8]);
          $lname = strtoupper($value[6]);
          $email = $value[2];
          $sex = $value[9];
          $nationality = $value[10];
          $provcode = $value[24];

          $suffix = strtoupper($value[20]);
          if (trim($suffix) == 'JR' || trim($suffix) == 'SR') {
            $suffix = $suffix.'.';
          }
          ...
          if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            if (!trim($fname) || !trim($lname) || !trim($sex) || !trim($schedule)) {
              $msg = 'Firstname, Surname, Gender, and Traing date fields are required';
               $msgclr = 'danger';
            }
          }else{
            $msg = 'Please check csv format.';
            $msgclr = 'danger';
          }

          
          if (checkdate(substr($schedule, 5, 2), substr($schedule, 8, 2), intval(substr($schedule, 0, 4)))) {
            ...
            $sql .= "('".$refstart."', '".$schedule."', AES_ENCRYPT('".$fname."', CONCAT('".$refstart."', 'PLANNER')), AES_ENCRYPT('".$mname."', CONCAT('".$refstart."', 'PLANNER')), AES_ENCRYPT('".$lname."', CONCAT('".$refstart."', 'PLANNER')), '".$value[9]."', AES_ENCRYPT('".$email."', CONCAT('".$refstart."', 'PLANNER')), AES_ENCRYPT('".$provcode."', CONCAT('".$refstart."', 'PLANNER')), '".date("Y-m-d H:i:s")."', '".$value[1]."', '".$value[3]."', '".addslashes($value[4])."', '".addslashes($value[5])."', '".$value[10]."', '".$value[11]."', '".$value[12]."', '".addslashes($value[13])."', '".$value[14]."', '".$value[16]."', '".$value[17]."', '".addslashes($value[18])."', '".addslashes($value[19])."', '".addslashes($value[23])."', AES_ENCRYPT('".$suffix."', CONCAT('".$refstart."', 'PLANNER'))), ";

            
          }else{
            $err_date = 1;
            $msg = 'Please use date format yyyy-mm-dd';
            $msgclr = 'danger';
          }
