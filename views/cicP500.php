...
if ($_GET['nid'] == 5 and $_GET['sid'] == 0 and $_GET['rid'] == 0) {
...
<!--             <th width="9%">Available Credits</th>
        ...
        <tbody>
         <?php
          ...
          $get_inquiry_month = $dbh->query("SELECT fld_provcode, SUM(fld_inqcount) as inqcnt, SUM(fld_inq_price) as inqprice FROM tbinquiries WHERE YEAR(fld_inqdate) = '".$det[0]."' and MONTH(fld_inqdate) = '".$det[1]."' AND (fld_branchcode = 'USERS' or fld_branchcode = 'BATCH') AND fld_inqresult = 1 GROUP BY fld_provcode");
