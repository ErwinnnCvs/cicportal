...
<!-- 
    ...
    <div class="row">
      <div class="col-lg-3">
            <div class="form-group">
                <button type="button" class="btn btn-primary" id="exportCSV" onclick="exportCSVBtn()">Export Contacts</button>
            </div>
        </div>
    </div>
    ...
    <br><br>
          ...
          if($_POST['entitytype'] != "all" and $_POST['entitytype'] and !$_POST['sel_assoc']){
          ...
          } elseif ($_POST['entitytype'] == 'all' and $_POST['sel_assoc']) {
            $query = " AND (a.fld_assoc_code = '".$_POST['sel_assoc']."')";
          } else {
            $query = "";
          }
          ...
          while ($gass=$get_all_seps_sub->fetch_array()) {
            if($_POST['compliancerating'] == 'all' || !$_POST['compliancerating'] || empty($_POST['compliancerating'])){
              ...
              if ($gass['fld_date'] and date("Y", strtotime($gass['fld_date'])) == 2023 ){
                $date1 = $gass['fld_date'];
              $date2 = '2023-12-31';
          ...
          $get_all_sep=$dbh4->query("SELECT a.fld_ctrlno, a.fld_type, AES_DECRYPT(a.fld_provcode, md5(CONCAT(a.fld_ctrlno, 'RA3019'))) as fld_provcode, AES_DECRYPT(a.fld_name, md5(CONCAT(fld_ctrlno, 'RA3019'))) as fld_name,
          AES_DECRYPT(a.fld_fname_ar, md5(CONCAT(fld_ctrlno, 'RA3019'))) as fld_fname_ar, AES_DECRYPT(a.fld_mname_ar, md5(CONCAT(a.fld_ctrlno, 'RA3019'))) as fld_mname_ar, AES_DECRYPT(a.fld_lname_ar, md5(CONCAT(fld_ctrlno, 'RA3019'))) as fld_lname_ar, AES_DECRYPT(a.fld_extname_ar, md5(CONCAT(fld_ctrlno, 'RA3019'))) as fld_extname_ar, AES_DECRYPT(a.fld_email_ar, md5(CONCAT(fld_ctrlno, 'RA3019'))) as fld_email_ar,
          AES_DECRYPT(a.fld_fname_c1, md5(CONCAT(fld_ctrlno, 'RA3019'))) as fld_fname_c1, AES_DECRYPT(a.fld_mname_c1, md5(CONCAT(fld_ctrlno, 'RA3019'))) as fld_mname_c1, AES_DECRYPT(a.fld_lname_c1, md5(CONCAT(fld_ctrlno, 'RA3019'))) as fld_lname_c1, AES_DECRYPT(a.fld_extname_c1, md5(CONCAT(fld_ctrlno, 'RA3019'))) as fld_extname_c1, AES_DECRYPT(a.fld_email_c1, md5(CONCAT(fld_ctrlno, 'RA3019'))) as fld_email_c1, b.fld_date FROM tbentities a LEFT JOIN tbsep b ON AES_DECRYPT(a.fld_provcode, md5(CONCAT(a.fld_ctrlno, 'RA3019'))) = b.fld_provcode  WHERE a.fld_ctrlno IN (".implode(",", $comrating).") and a.fld_registration_type <> 1".$query." OR a.fld_ctrlno IN (".implode(",", $comrating).") and (a.fld_registration_type = 1 AND a.fld_noc_pass_status = 1".$query." )");
          ...
          while($gap=$get_all_sep->fetch_array()){
            ...
            $export_caf_arr[$gap['fld_provcode']] = array(
              "ENTITY_NAME"=>$gap['fld_name'], "ENTITY_TYPE"=>$gap['fld_type'], "DATE_ONBOARDED"=>$date_onboarded, "MISSED_MONTHS"=>$missed_months_submitted,
              "COMPLIANCE_RATING"=>$compliance_rating, "RECORD_TYPE"=>$ctnla_label, "AVERAGE_SUBMITTED_COUNT"=>$submitted_count, "FLAT_RATE"=>200000, "EXCESS_100K"=>$excess_record, "VARIABLE_RATE"=>$variable_rate,
              "GROSS_CAF"=>$gross_caf, "REDUCTION_RATE"=>$reduction_fee, "NET_CAF"=>$new_fee, "AR_FNAME"=>$gap['fld_fname_ar'], "AR_LNAME"=>$gap['fld_lname_ar'], "AR_EXTNAME"=>$gap['fld_extname_ar'], "AR_EMAIL"=>$gap['fld_email_ar'], "PRIMARY_FNAME"=>$gap['fld_fname_c1'], "PRIMARY_MNAME"=>$gap['fld_mname_c1'], "PRIMARY_LNAME"=>$gap['fld_lname_c1'], "PRIMARY_EXTNAME"=>$gap['fld_extname_c1'], "PRIMARY_EMAIL"=>$gap['fld_email_c1']
            );
          }
          echo "</tbody>";
          ...
          ?>
          <input type="hidden" name="caf_contacts_array" id="caf_contacts_array" value="<?php echo htmlspecialchars(json_encode($export_caf_arr)); ?>">
        </div>
        <!-- /.box-body -->
      </div>
      <!-- /.box -->
...
<script type="text/javascript">
  function exportCSVBtn() {
    ...
    var caf = document.getElementById('caf_contacts_array').value;
    $.ajax({
         type: 'POST',
         url: 'ajax/cafextractcontacts.php',
         data: {caf : caf, datetime : timestamp},
         success: function(response){
          window.location.href = "downloadfile_caf_contacts.php?file=" + response;
         }
      });
  }
  // 
