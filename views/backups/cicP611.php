...
$get_beginning_balance = $dbh->query("SELECT fld_converted_credits FROM tbbillingbalance WHERE fld_provcode = '".$sae."' and YEAR(fld_stmt_date) = '".$det[0]."' and MONTH(fld_stmt_date) = '".$det[1]."' ");
...
$current_balance = ($gbb['fld_converted_credits'] + $gdm['amount']) - $gcim['inqprice'];
        ...
        <tbody>
         ...
         #CIBI
          ...
          while($gcim=$get_cibi_inquiry_month->fetch_array()){
            ...
            $get_access_limit = $dbh->query("SELECT fld_accountno FROM tbbilling WHERE fld_provcode = '".$sae_provider_code."'");
            $gal=$get_access_limit->fetch_array();

            $get_beginning_balance = $dbh->query("SELECT fld_converted_credits FROM tbbillingbalance WHERE fld_provcode = '".$sae_provider_code."' and YEAR(fld_stmt_date) = '".$det[0]."' and MONTH(fld_stmt_date) = '".$det[1]."' ");
            $gbb=$get_beginning_balance->fetch_array();



            $get_deposit_month = $dbh->query("SELECT SUM(fld_converted_credits) as credit_amount FROM tbcrbillingpayment WHERE fld_acct_no = '".$gal['fld_accountno']."' and fld_datetime LIKE '".$det[0]."-".$det[1]."%' and fld_transaction_type <> 'MIGRATED' GROUP BY fld_acct_no");
            $gdm=$get_deposit_month->fetch_array();
            ...
            $get_next_beginning_balance_cb = $dbh->query("SELECT fld_chargeback FROM tbbillingbalance WHERE fld_provcode = '".$sae_provider_code."' and fld_stmt_date = '".$date1." '");
            $gnbbcb=$get_next_beginning_balance_cb->fetch_array();

            $beggining_cb_balance = $dbh->query("SELECT fld_chargeback FROM tbbillingbalance WHERE fld_provcode = '".$sae_provider_code."' and fld_stmt_date = '".$det[0]."-".$det[1]."-01"."'");
