...
if (!$_GET['fstatus']) {
  ...
  if ($_GET['fstatus'] == "1") {
   ...
   $query = "SELECT fld_ctrlno, AES_DECRYPT(fld_name, MD5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_name, AES_DECRYPT(fld_provcode, MD5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_provcode, fld_ds_update_stats, fld_type, fld_ds_update_ts, fld_re_approval_rej_ts, fld_status FROM tbentities WHERE fld_process_status = 2 and fld_re_validation_status = 1 and fld_re_approval_status = 2 and fld_re_approval_rej_ts IS NOT NULL and fld_operational = 0 and fld_registration_type = 1 and AES_DECRYPT(fld_provcode, md5(CONCAT(fld_ctrlno, 'RA3019'))) NOT LIKE 'SAE%';";
  } elseif ($_GET['fstatus'] == "3") {
    $current_status = "Approved";
    $query = "SELECT fld_ctrlno, AES_DECRYPT(fld_name, MD5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_name, AES_DECRYPT(fld_provcode, MD5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_provcode, fld_ds_update_stats, fld_type, fld_ds_update_ts, fld_re_approval_ts, fld_status FROM tbentities WHERE fld_process_status = 3 and fld_re_validation_status = 1 and fld_re_approval_status = 1 and fld_re_approval_ts IS NOT NULL and fld_operational = 0 and fld_registration_type = 1 and AES_DECRYPT(fld_provcode, md5(CONCAT(fld_ctrlno, 'RA3019'))) NOT LIKE 'SAE%';";
  } else {
    $current_status = "Pending";
    $query = "SELECT fld_ctrlno, AES_DECRYPT(fld_name, MD5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_name, AES_DECRYPT(fld_provcode, MD5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_provcode, fld_ds_update_stats, fld_type, fld_ds_update_ts, fld_re_validation_ts, fld_status FROM tbentities WHERE fld_process_status = 3 and fld_re_validation_status = 1 and fld_re_approval_status <= 1 and fld_re_approval_ts IS NULL and fld_operational = 0 and fld_registration_type = 1 and AES_DECRYPT(fld_provcode, md5(CONCAT(fld_ctrlno, 'RA3019'))) NOT LIKE 'SAE%';";
  }
          ...
          <table class="table table-head-fixed text-nowrap">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><a href="main.php?nid=4&sid=1&rid=1&ctrlno=<?php echo $gae['fld_ctrlno']; ?>"><?php echo $gae['fld_name']; ?></a></td>
