        ...
        <tbody>
         <?php
          ...
          while($gim=$get_inquiry_month->fetch_array()){
            ...
            $get_beginning_balance = $dbh->query("SELECT fld_beginbalance FROM tbbillingbalance WHERE fld_provcode = '".$gim['fld_provcode']."' and YEAR(fld_stmt_date) = '".$det[0]."' and MONTH(fld_stmt_date) = '".$det[1]."' ");
            $gbb=$get_beginning_balance->fetch_array();



            $get_deposit_month = $dbh->query("SELECT SUM(fld_amount) as amount FROM tbbillingpayment WHERE fld_acct_no = '".$gal['fld_accountno']."' and fld_datetime LIKE '".$det[0]."-".$det[1]."%' GROUP BY fld_acct_no");
            ...
            if ($det[0]."-".$det[1] == date("Y-m", strtotime($gnts['fld_aeis_noc_ts']))) {
              ...
              $get_initial_payment = $dbh->query("SELECT SUM(fld_amount) as fld_amount FROM tbbillingpayment WHERE fld_acct_no = '".$gal['fld_accountno']."'");
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <?php
                      $get_all_payments = $dbh->query("SELECT fld_amount as amount, fld_datetime as pdate FROM tbbillingpayment WHERE fld_acct_no = '".$gal['fld_accountno']."' and fld_charge_back = 0 and fld_published = 1 ORDER BY fld_payment_date DESC");
                  ...
                  <?php
                    ...
                    $get_total_payments = $dbh->query("SELECT SUM(fld_amount) as amount FROM tbbillingpayment WHERE fld_acct_no = '".$gal['fld_accountno']."' and fld_charge_back = 0 and fld_published = 1 GROUP BY fld_acct_no");
