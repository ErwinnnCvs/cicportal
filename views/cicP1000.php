          ...
          <table class="table table-head-fixed text-nowrap">
            ...
            <tbody>
              <?php
                ...
                //     $datetime = date("F d, Y h:ia", strtotime($gae['fld_re_validation_ts']));
                //   }
                $get_entity_data_purging = $dbh4->query("SELECT fld_ctrlno, aes_decrypt(fld_name, md5(CONCAT(fld_ctrlno, 'RA3019'))) as name, aes_decrypt(fld_provcode, md5(CONCAT(fld_ctrlno, 'RA3019'))) as provcode, fld_type, fld_dqua_sae_report_ts FROM tbentities WHERE fld_dqua_sae_report_status = 1 and fld_dqua_report_status = 0");
                while ($gedp=$get_entity_data_purging->fetch_array()) {
                  $datetime = date("F d, Y H:ia", strtotime($gedp['fld_dqua_sae_report_ts']));
              ?>
              <tr>
                <td><?php echo $gedp['provcode']; ?></td>
                <td><a href="main.php?nid=10&sid=1&rid=1&ctrlno=<?php echo $gedp['fld_ctrlno']; ?>"><?php echo $gedp['name']; ?></a></td>
                <td><?php echo $ent2[$gedp['fld_type']]; ?></td>
