...
if ($_POST['sbtSave']) {
  if($dbh4->query("UPDATE tbassign SET fld_active = 0 WHERE fld_provcode = '".$_POST['sbtSave']."' and fld_type = 2")){
    if ($dbh4->query("INSERT INTO tbassign (fld_provcode, fld_assign, fld_by) VALUES ('".$_POST['sbtSave']."', '".$_POST['assignse']."', '".$_SESSION['name']."', 2)")) {
      echo "UPDATED";
    } else {
      echo "ERROR";
    }
    ...
    <div class="card-body">
      <table class="table table-bordered table-hover" id="unassignedse">
        ...
        <tbody>
          <?php
            $c = 1;
            $get_all_seps_sub=$dbh4->query("SELECT a.fld_ctrlno, a.fld_type, AES_DECRYPT(a.fld_provcode, md5(CONCAT(a.fld_ctrlno, 'RA3019'))) as fld_provcode, AES_DECRYPT(a.fld_name, md5(CONCAT(fld_ctrlno, 'RA3019'))) as fld_name, b.fld_date FROM tbentities a RIGHT JOIN tbsep b ON AES_DECRYPT(a.fld_provcode, md5(CONCAT(a.fld_ctrlno, 'RA3019'))) = b.fld_provcode  WHERE fld_assignsep_legal = 1 AND (a.fld_registration_type <> 1 OR (a.fld_registration_type = 1 AND a.fld_noc_pass_status = 1))");
            while ($gass=$get_all_seps_sub->fetch_array()) {
              $get_assignsep_table = $dbh4->query("SELECT * FROM tbassign WHERE fld_provcode = '".$gass['fld_provcode']."' and fld_active = 1 and fld_type = 2 ORDER BY fld_id DESC LIMIT 1");
          ...
          <tr>
            ...
            <td>
              <button type="button" class="btn btn-success btn-block" data-toggle="modal" data-target="#modal-default<?php echo $gass['fld_provcode']; ?>">
              <?php
               echo $gu['fld_name']; 
              ?>
              </button>
