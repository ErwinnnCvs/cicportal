...
if ($ged['fld_ctrlno']) {
  ...
  if (!$_POST['sbtCommentSave'] and !$_POST['sbtCommentCompleted'] and !$_POST['sbtStartCall'] and !$_POST['sbtEndCall'] and !$_POST['sbtSaveCallLogs']) {
    $actTab[1] = " active";
    $tabnavlink[1] = " active";
  }

  if ($_POST['sbtCommentSave'] || $_POST['sbtCommentCompleted']) {
    $actTab[1] = " active";
    $tabnavlink[1] = " active";
  } elseif ($_POST['sbtStartCall'] || $_POST['sbtEndCall'] || $_POST['sbtSaveCallLogs']) {
    $actTab[2] = " active";
    $tabnavlink[2] = " active";
  }
  ...
  if ($_POST['sbtCommentSave']) {
    $txt = addslashes($_POST['commentTxt']);

    if ($dbh->query("INSERT INTO tbrevalidation_remarks (fld_ctrlno, fld_validation, fld_status, fld_comments, fld_ts, fld_comment_by) VALUES ('".$controlno."', '1', '0', '".$txt."', '".$timestamp."', '".$_SESSION['name']."')")) {
      if ($dbh4->query("UPDATE tbentities set fld_re_validation_status = 2, fld_re_validation_incom_ts = '".$timestamp."', fld_re_validation_by = '".$_SESSION['name']."' WHERE fld_ctrlno = '".$controlno."'; ")) {
        if ($dbh4->query("INSERT INTO tbupdatesaudit (fld_ctrlno, fld_data, fld_form, fld_user, fld_ts, fld_source) VALUES ('".$controlno."', 'AES_ENCRYPT(".$txt."', md5(CONCAT(fld_ctrlno, 'RA3019'))), 'REVINC', '".$_SESSION['user_id']."', '".$timestamp."', '".$audit_source."')")) {
  ...
  } elseif ($_POST['sbtCommentSaveSend']) {
    $txt = addslashes($_POST['commentTxt']);

    if ($dbh->query("INSERT INTO tbrevalidation_remarks (fld_ctrlno, fld_validation, fld_status, fld_comments, fld_ts, fld_comment_by) VALUES ('".$controlno."', '2', '0', '".$txt."', '".$timestamp."', '".$_SESSION['name']."')")) {
      if ($dbh4->query("UPDATE tbentities set fld_re_validation_status = 2, fld_re_validation_incom_ts = '".$timestamp."', fld_re_validation_by = '".$_SESSION['name']."' WHERE fld_ctrlno = '".$controlno."'; ")) {
        if ($dbh4->query("INSERT INTO tbupdatesaudit (fld_ctrlno, fld_data, fld_form, fld_user, fld_ts, fld_source) VALUES ('".$controlno."', 'AES_ENCRYPT(".$txt."', md5(CONCAT(fld_ctrlno, 'RA3019'))), 'REVDISC', '".$_SESSION['user_id']."', '".$timestamp."', '".$audit_source."')")) {
          $msg = "Successfully updated";
          $msgclr = "success";
        }
      }
      # incomplete and send email

    }
  } elseif ($_POST['sbtCommentCompleted']) {
    $txt = addslashes($_POST['commentTxt']);

    if ($dbh->query("INSERT INTO tbrevalidation_remarks (fld_ctrlno, fld_validation, fld_status, fld_comments, fld_ts, fld_comment_by) VALUES ('".$controlno."', '3', '1', '".$txt."', '".$timestamp."', '".$_SESSION['name']."')")) {
      if ($dbh4->query("UPDATE tbentities set fld_re_validation_status = 1, fld_re_validation_ts = '".$timestamp."', fld_re_validation_by = '".$_SESSION['name']."', fld_process_status = 3 WHERE fld_ctrlno = '".$controlno."'; ")) {
        if ($dbh4->query("INSERT INTO tbupdatesaudit (fld_ctrlno, fld_data, fld_form, fld_user, fld_ts, fld_source) VALUES ('".$controlno."', 'AES_ENCRYPT(".$txt."', md5(CONCAT(fld_ctrlno, 'RA3019'))), 'REVCOM', '".$_SESSION['user_id']."', '".$timestamp."', '".$audit_source."')")) {
          $msg = "Successfully updated";
          $msgclr = "success";
        }
      }
...
<section class="content">
  ...
  <div class="row">
    ...
    <div class="col-6">
      ...
      <?php
        if ($ged['fld_re_validation_status'] == 0 || $ged['fld_re_validation_status'] == 1) {
      ?>
      <div class="card">
        <div class="card-header d-flex p-0">
          <h3 class="card-title p-3">Action</h3>
          <ul class="nav nav-pills ml-auto p-2">
            <li class="nav-item"><a class="nav-link<?php echo $tabnavlink[1]; ?>" href="#tab_1" data-toggle="tab">Comments</a></li>
            <li class="nav-item"><a class="nav-link<?php echo $tabnavlink[2]; ?>" href="#tab_2" data-toggle="tab">Call</a></li>
          </ul>
        </div><!-- /.card-header -->
        <div class="card-body">
          <form method="post">
            <div class="tab-content">
              <div class="tab-pane<?php echo $actTab[1]; ?>" id="tab_1">
                <div class="form-group">
                  <label>Validation</label>
                  <select class="form-control" name="selValidationOptions" onchange="submit()">
                ...
                <br>
                <?php
                  if ($_POST['selValidationOptions'] == 1) {
                ?>
                <button type="submit" name="sbtCommentSave" value="1" class="btn btn-default">Save</button>
                <?php
                  } elseif ($_POST['selValidationOptions'] == 2) {
                ?>
                <button type="submit" name="sbtCommentSaveSend" value="1" class="btn btn-primary">Save and Send Email to AR</button>
                <?php
                  } elseif ($_POST['selValidationOptions'] == 3) {
                ?>
                <button type="submit" name="sbtCommentCompleted" value="1" class="btn btn-success float-right">Save and Complete</button>
                <?php
                  }
                ?>
              </div>
              <!-- /.tab-pane -->
              <div class="tab-pane<?php echo $actTab[2]; ?>" id="tab_2">
                <div class="row">
                  <div class="col-sm-8 col-md-8">
                    <div class="form-group row">
                      <label for="call-name" class="col-sm-2 col-form-label">Name</label>
                      <div class="col-sm-8">
                        <input type="text" class="form-control" name="call-name" id="call-name" placeholder="">
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="call-contact" class="col-sm-2 col-form-label">Contact No:</label>
                      <div class="col-sm-8">
                        <input type="text" class="form-control" name="call-contact" id="call-contact" placeholder="">
                      </div>
                    </div>
                  </div>
                  <div class="col-sm-4 col-md-4">
                    <button type="submit" name="sbtStartCall" value="1" class="btn btn-success btn-block">Start Call</button>
                    <button type="submit" name="sbtEndCall" value="1" class="btn btn-danger btn-block">End Call</button>
                  </div>
                </div>
                <label>Remarks:</label>
                <textarea class="form-control"></textarea>
                <br>
                <button type="submit" class="btn btn-success float-right" value="1" name="sbtSaveCallLogs">Save</button>
              </div>
              <!-- /.tab-pane -->
            </div>
            <!-- /.tab-content -->
