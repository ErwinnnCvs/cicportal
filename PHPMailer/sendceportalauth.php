...
require_once $_SERVER['DOCUMENT_ROOT'].'/cicportal/PHPMailer/PHPMailerAutoload.php';
$key = 'RA3019';
$sql2=$dbh4->query("SELECT fld_ctrlno, AES_DECRYPT(fld_name, md5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_name, AES_DECRYPT(fld_lname_ar,md5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_lname_ar, AES_DECRYPT(fld_fname_ar,md5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_fname_ar, AES_DECRYPT(fld_mname_ar,md5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_mname_ar, AES_DECRYPT(fld_extname_ar,md5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_extname_ar, AES_DECRYPT(fld_provcode,md5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_provcode, AES_DECRYPT(fld_email_ar,md5(CONCAT(fld_ctrlno, '".$key."'))) AS fld_email_ar FROM tbentities WHERE fld_ctrlno = '".$controlno."'");

$s=$sql2->fetch_array();
$email = $s['fld_email_ar'];
$name = $s['fld_fname_ar']. " " .substr($s['fld_mname_ar'], 0, 1). " " .$s['fld_lname_ar']. " " .$s['fld_extname_ar'];
$company = $s['fld_name'];
...
$c=$credentials->fetch_array();
        ...
        // $mail->addCc("gil.escalante@creditinfo.gov.ph", "Jose Gil Escalante");
                     ...
                     <tbody>
                        <tr height="20px" rowspan="1" colspan="3">
                           ...
                           <p>Dear <b>'.$name.'</b>,<br/></p>
                            ...
                            <p align="justify">This is to inform you that <b>'.$company.'</b> already completed the online Submitting Entity Information Sheet (SEIS) and may now register as an Accessing Entity of the Credit Information Corporation (CIC). We have released the registration link to your registered Primary Contact Person to continue the registration.</p>
                            ...
                            <p><b>Below are the payment channels of the CIC with Account No. 1802-1033-91:</b>
                                <ol>
                                    <li type="1" align="justify">Land Bank of the Philippines (LBP) or WeAccess Facility, if applicable;</li>
