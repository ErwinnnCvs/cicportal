...
$pdf = new MYPDF3('P','mm',array(210, 297));
...
$pdf->AddPage();
$pdf->SetProtection(array('modify'), trim($res['provcode']));
$pdf->Ln(20);
$pdf->SetFont('times', 'B', 11);
$pdf->MultiCell(15, 5, "To", 0, 'L', 0, 0, '', '', true);
$pdf->MultiCell(15, 5, ":", 0, 'L', 0, 0, '', '', true);
$pdf->MultiCell(0, 5, "Special Accessing Entities", 0, 'L', 0, 1, '', '', true);
$pdf->MultiCell(15, 5, "Re", 0, 'L', 0, 0, '', '', true);
$pdf->MultiCell(15, 5, ":", 0, 'L', 0, 0, '', '', true);
$pdf->MultiCell(0, 5, "Certification Requirement for Special Accessing Entity’s Access to\nCredit Data on Behalf of a Submitting Entity", 0, 'L', 0, 1, '', '', true);

$pdf->MultiCell(0, 5, "", 'B', 'L', 0, 1, '', '', true);
$pdf->Ln(2);
$pdf->SetFont('times', '', 11);
// $pdf->Ln(1);
// $pdf->MultiCell(0, 5, "          The Special Accessing Entities (SAE) of the Credit Information Corporation (CIC) namely CRIF Philippines, COMPUSCAN Philippines, TransUnion Information Solutions and CIBI Information Inc., as part of the latter's services offered, will access the CIC's database via the Credit Information System, on behalf of their respective client Submitting Entities (SE).\n", 0, 'J', 0, 1, '', '', true);
$pdf->SetX(35);
...
$pdf->SetX(35);
$pdf->MultiCell(5, 5, "8.\n", 0, 'J', 0, 0, '', '', true);
$pdf->MultiCell(0, 5, "If the contract between the SAE and SE is time-bound, the validity of the authority shall terminate upon the expiration of the term in the contract. Both the SAE and the SE must inform the CIC that their contract has been terminated; and\n", 0, 'J', 0, 1, '', '', true);
$pdf->SetX(35);
$pdf->MultiCell(5, 5, "9.\n", 0, 'J', 0, 0, '', '', true);
$pdf->MultiCell(0, 5, "The CIC assumes no liability for the SAE on its unauthorized access in the event that the CIC has not been informed of the termination of the contract. Unauthorized access of the data from the CIC is punishable by law.\n", 0, 'J', 0, 1, '', '', true);
...
$pdf->Ln(6);
$pdf->MultiCell(80, 5, "          ______________________________          ", 0, 'C', 0, 0, '', '', true);
$pdf->MultiCell(80, 5, "          ______________________________          ", 0, 'C', 0, 1, '', '', true);
$pdf->MultiCell(80, 5, "Name of Submitting Entity\n", 0, 'C', 0, 0, '', '', true);
$pdf->MultiCell(80, 5, "Name of Special Accessing Entity\n", 0, 'C', 0, 1, '', '', true);
// $pdf->Ln(1);
...
$pdf->MultiCell(60, 5, "", 'B', 'C', 0, 0, '', '', true);
$pdf->Write(0, '            By:     ', '', 0, 'L', 0, 0, false, false, 0);
$pdf->MultiCell(60, 5, "", 'B', 'C', 0, 1, '', '', true);
$pdf->SetFont('times', 'B', 9);
$pdf->MultiCell(80, 5, "SE REPRESENTATIVE\n", 0, 'C', 0, 0, '', '', true);
$pdf->MultiCell(80, 5, "SAE REPRESENTATIVE\n", 0, 'C', 0, 1, '', '', true);
$pdf->SetFont('times', '', 9);
$pdf->MultiCell(80, 5, "Authorized Representative\n", 0, 'C', 0, 0, '', '', true);
$pdf->MultiCell(80, 5, "Authorized Representative\n", 0, 'C', 0, 1, '', '', true);
$pdf->SetFont('times', 'I', 9);
$pdf->MultiCell(80, 5, "Signature over Printed Name", 0, 'C', 0, 0, '', '', true);
$pdf->MultiCell(80, 5, "Signature over Printed Name\n", 0, 'C', 0, 1, '', '', true);
...
$pdf->SetY(122);
$pdf->SetFont('times', 'B', 10);
$pdf->MultiCell(0, 5, $sae['name'], 0, 'C', 0, 0, '', '', true);

$pdf->SetY(127);
$pdf->MultiCell(0, 5, $res['name'], 0, 'C', 0, 0, '', '', true);
$pdf->SetY(228);
$pdf->MultiCell(80, 5, $res['name'], 0, 'C', 0, 0, '', '', true);
$pdf->MultiCell(80, 5, $sae['name'], 0, 'C', 0, 0, '', '', true);

$pdf->SetY(246);
$pdf->MultiCell(80, 5, utf8_encode($res['fname_ar'].($res['mname_ar']? " ".substr($res['mname_ar'], 0, 1).'.':'')." ".$res['lname_ar'].($res['extname_ar']? " ".$res['extname_ar']:'')), 0, 'C', 0, 0, '', '', true);
...
$pdf->MultiCell(0, 5, "Upon sending of the completely filled up and signed Certification-request via e-mail to", 0, 'J', 0, 1, '', '', true);// datasubmission@creditinfo.gov.ph, it shall be reviewed by the Data Submission Team for not more than three (3) working days.\n\nThe e-mail header requesting access on behalf of the SE shall be in the following format:
...
$pdf->Ln(20);
$pdf->SetFont('times', 'B', 11);
$pdf->MultiCell(0, 5, "SAE ACCESS REQUEST FOR [name of SAE] ON\n BEHALF OF [name of SE]", 0, 'C', 0, 1, '', '', true);
$pdf->SetFont('times', '', 11);
$pdf->Ln(2);
...
// $pdf->Ln(5);
// $pdf->SetX(35);
$pdf->Image('images/verification.png', 35, 160, 140, 115, '', '', '', false, 300, '', false, false, 0);

$pdf->lastPage();
$pdf->Output('../pdf/AGR_'.$res['provcode'].'_'.$sae['provcode'].'_TOSAE.pdf', 'F');
